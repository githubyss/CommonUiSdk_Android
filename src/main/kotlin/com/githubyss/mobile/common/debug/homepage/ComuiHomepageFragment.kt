...
class ComuiHomepageFragment : BaseFragment() {
    ...
    private val onClickListener = View.OnClickListener { v ->
        when (v.id) {
            R.id.button_propertyAnimation -> replaceFragment(
                ComuiPropertyAnimationFragment(),
                ComuiPropertyAnimationFragment.TAG,
                true
            )
            R.id.button_tweenAnimation -> return@OnClickListener
            R.id.button_recyclerView -> replaceFragment(
                ComuiRecyclerViewFragment(),
                ComuiRecyclerViewFragment.TAG,
                true
            )
            ...
            R.id.button_showFloatingWithinApp -> {
                ...
                layoutParams.setMargins(
                    ...
                    (ScreenUtils.dp2Px(14.0f) ?: return@OnClickListener),
                    (ScreenUtils.dp2Px(14.0f) ?: return@OnClickListener)
                )
                AppFloatingIcon.getInstance(activity as Context).show()?.layoutParams(layoutParams)
                    ?.setMovable(true)
            ...
            R.id.button_showFloatingWithinSystem -> {
                SystemFloatingIcon.getInstance(activity as Context).show()?.setMovable(true)
                    ?.customIcon(R.drawable.comui_lucky_money)
                    ?.setFloatingListener(object : FloatingIconListener {
                        override fun onShow() {}
                        override fun onRemove() {}
            ...
            R.id.button_showAutoShortedFloatingView -> {
                ...
                audioList.add(
                    AudioModel(
                        ...
                        "",
                        "",
                        "http://ossprexg.cnsuning.com/fipcms/media/voice/male/1608360545089.wav",
                        "http://ossprexg.cnsuning.com/fipcms/media/voice/female/1608360545089.wav"
                    )
                )
                audioList.add(
                    AudioModel(
                        ...
                        "",
                        "",
                        "http://ossprexg.cnsuning.com/fipcms/media/voice/male/160877400382021.wav",
                        "http://ossprexg.cnsuning.com/fipcms/media/voice/female/160877400382021.wav"
                    )
                )
                ...
                AppFloatingAudioPlayer.getInstance(activity as Context).show()
                    ?.setAppFloatingForNativeListener(object : FloatingAudioPlayerListener {
                        ...
                        override fun onUpdateAudioInfo(audioModel: AudioModel?) {
                            run {
                                LogcatUtils.d(
                                    TAG,
                                    "FloatingAudioPlayerListener >>> onUpdateAudioInfo >> audioModel: $audioModel"
                                )
